//------------------------------------------------------------------------------
//  mrt-d3d12.c
//  Multiple-render-target rendering with the D3D12 backend.
//------------------------------------------------------------------------------
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow) {
    ...
    d3d12_init(&(d3d12_desc_t){ .width = width, .height = height, .title = L"mrt-d3d12.c" });
    sg_setup(&(sg_desc){
    ...
    while (d3d12_process_events()) {
        ...
        const offscreen_params_t offscreen_params = {
            .mvp = compute_mvp(rx, ry, d3d12_width(), d3d12_height()),
        };
