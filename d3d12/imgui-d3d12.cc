//------------------------------------------------------------------------------
//  imgui-d3d12.cc
//  Dear ImGui integration sample with D3D12 backend.
//------------------------------------------------------------------------------
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow) {
    ...
    // setup d3d12 app wrapper, sokol_gfx, sokol_time
    d3d12_desc_t d3d12_desc = {};
    d3d12_desc.width = Width;
    d3d12_desc.height = Height;
    d3d12_desc.title = L"imgui-d3d12.c";
    d3d12_init(&d3d12_desc);
    sg_desc desc = { };
    desc.environment = d3d12_environment();
    desc.logger.func = slog_func;
    sg_setup(&desc);
    stm_setup();

    // input forwarding
    d3d12_mouse_pos([] (float x, float y)   { ImGui::GetIO().AddMousePosEvent(x, y); });
    d3d12_mouse_btn_down([] (int btn)       { ImGui::GetIO().AddMouseButtonEvent(btn, true); });
    d3d12_mouse_btn_up([] (int btn)         { ImGui::GetIO().AddMouseButtonEvent(btn, false); });
    d3d12_mouse_wheel([](float v)           { ImGui::GetIO().AddMouseWheelEvent(0, v); });
    d3d12_char([] (wchar_t c)               { ImGui::GetIO().AddInputCharacter(c); });
    d3d12_key_down([] (int key)             { ImGui::GetIO().AddKeyEvent(as_imgui_key(key), true); });
    d3d12_key_up([] (int key)               { ImGui::GetIO().AddKeyEvent(as_imgui_key(key), false); });
    ...
    while (d3d12_process_events()) {
        const int cur_width = d3d12_width();
        const int cur_height = d3d12_height();
        ...
        pass.swapchain = d3d12_swapchain();
        sg_begin_pass(&pass);
        ImGui::Render();
