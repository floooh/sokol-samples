...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow) {
    ...
    d3d12_init(&(d3d12_desc_t){ .width = 800, .height = 600, .sample_count = 4, .title = L"blend-d3d12.c" });
    sg_setup(&(sg_desc){
    ...
    while (d3d12_process_events()) {
        const mat44_t proj = mat44_perspective_fov_rh(vm_radians(90.0f), (float)d3d12_width()/(float)d3d12_height(), 0.01f, 100.0f);
