...
static void draw_scene_1(const ImDrawList* dl, const ImDrawCmd* cmd) {
    ...
    const mat44_t rxm = mat44_rotation_x(vm_radians(state.scene1.rx));
    const mat44_t rym = mat44_rotation_y(vm_radians(state.scene1.ry));
...
static void frame(void) {
    ...
    if (igBegin("Dear ImGui", 0, 0)) {
        if (igBeginChild("sokol-gfx", (ImVec2){360, 360}, ImGuiChildFlags_Borders, ImGuiWindowFlags_None)) {
            ImDrawList* dl = igGetWindowDrawList();
            ImDrawList_AddCallback(dl, draw_scene_1);
        }
        igEndChild();
        igSameLineEx(0, 10);
        if (igBeginChild("sokol-gl", (ImVec2){360, 360}, ImGuiChildFlags_Borders, ImGuiWindowFlags_None)) {
            ImDrawList* dl = igGetWindowDrawList();
...
sapp_desc sokol_main(int argc, char* argv[]) {
    ...
    return (sapp_desc) {
        ...
        .width = 860,
        .height = 440,
        .window_title = "imgui-usercallback-sapp.c",
