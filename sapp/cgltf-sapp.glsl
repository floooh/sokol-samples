...
void main() {
    ...
    // Roughness is stored in the 'r' channel, metallic is stored in the 'a' channel.
    // (same idea as storing normal maps in a DXT5N tetxure)
    vec4 mr_sample = texture(metallic_roughness_texture, v_uv);
    float perceptual_roughness = clamp(mr_sample.r * roughness_factor, 0.0, 1.0);
    float metallic = clamp(mr_sample.a * metallic_factor, 0.0, 1.0);
